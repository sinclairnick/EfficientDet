...
class SpatialPyramidPooling(keras.layers.Layer):
    ...
    def call(self, x, mask=None):
        ...
        row_length = [keras.backend.cast(num_rows, 'float32') / i for i in self.pool_list]
        col_length = [keras.backend.cast(num_cols, 'float32') / i for i in self.pool_list]
        ...
        for pool_num, num_pool_regions in enumerate(self.pool_list):
            for jy in range(num_pool_regions):
                for ix in range(num_pool_regions):
                    ...
                    x1 = keras.backend.cast(keras.backend.round(x1), 'int32')
                    x2 = keras.backend.cast(keras.backend.round(x2), 'int32')
                    y1 = keras.backend.cast(keras.backend.round(y1), 'int32')
                    y2 = keras.backend.cast(keras.backend.round(y2), 'int32')

                    new_shape = [y2 - y1,
                                    x2 - x1, input_shape[3]]

                    x_crop = x[:, y1:y2, x1:x2, :]
                    xm = keras.layers.Reshape(new_shape)(x_crop)
                    pooled_val = keras.backend.max(xm, axis=(1, 2))
                    outputs.append(pooled_val)


        outputs = keras.backend.concatenate(outputs, axis=1)
        outputs = keras.layers.Reshape((self.num_outputs_per_channel, self.nb_channels))(outputs)
